...
describeFork('Mainnet deployment fork smoke tests', (provider) => {
    ...
    before(async () => {
        ...
        if (!ENV.DEPLOYED) {
            ...
            const deployed = require(
                `../../../deploy/deployed-contracts/${ENV.DEPLOYED}.json`
            );
            addresses = {
    ...
    describeSnapshot('AuctionCrowdfund', provider, () => {
        ...
        describeSnapshot('zora', provider, () => {
            before(async () => {
                ...
                await mineTx(dummyERC721Contract.connect(seller)
                    .setApprovalForAll(zora.address, true));
                let r = await mineTx(zora.connect(seller).createAuction(
                    buyTokenId,
                ...
                auctionId = findEvent(
                    ...
                    'AuctionCreated',
                    zora.address,
                    { tokenContract: dummyERC721Contract.address, tokenId: buyTokenId },
                ).args.auctionId;

                r = await mineTx(crowdfundFactory.createAuctionCrowdfund(
                    {
                        name: PARTY_NAME,
                        symbol: PARTY_SYMBOL,
    ...
    describeSnapshot('CollectionBuyCrowdfund', provider, () => {
        ...
        before(async () => {
            buyTokenId = testERC721tokenIds[0];
            listing = await createOpenseaListing(
                dummyERC721Contract,
                buyTokenId,
                LIST_PRICE,
            );
            let r = await mineTx(crowdfundFactory.createCollectionBuyCrowdfund(
                {
                    name: PARTY_NAME,
                    symbol: PARTY_SYMBOL,
                    customizationPresetId: 0,
        ...
        describeSnapshot('winning path', provider, () => {
            it('can win', async () => {
                ...
                let r = await mineTx(cf.connect(host).buy(
                    buyTokenId,
                    opensea.address,
                    LIST_PRICE,
    ...
    describeSnapshot('BuyCrowdfund', provider, () => {
        ...
        describeSnapshot('winning path with governance', provider, () => {
            ...
            itSnapshot('can sell a governance NFT on OpenSea', provider, async () => {
                ...
                expect(doesEventExist(
                    r,
                    'Transfer',
                    party.address,
                    { owner: members[0].wallet.address, to: buyer.address },
                )).to.be.true;
                ...
                expect(await party['getVotingPowerAt(address,uint40)'](buyer.address, timestamp)).to.eq(token.votingPower);
            });

            it('can mint a token with arbitrary calls', async () => {
                nonPreciousTokenId =
                    await runArbitraryCallToMintTokenTest(party, members, buyTokenId);
            });
    ...
    async function runArbitraryCallToMintTokenTest(
        ...
        const { tokenId } = findEvent(
            r,
            'Transfer',
            dummyERC721Contract.address,
            { owner: NULL_ADDRESS, to: party.address },
        ).args;
        return tokenId;
    }
    ...
    async function runListOnOpenseaTest(
        ...
        noise(`Testing ${passUnanimously ? '(unanimous) ' : ''}opensea proposal on tokenId ${tokenId.toNumber()}...`);
    ...
    async function runDistributionTest(
        ...
        const r = await mineTx(daoWallet.execCall(
            distributor.address,
            ZERO,
            (await distributor.populateTransaction.claimFee(distInfo, daoWallet.address)).data,
        ));
    ...
    async function createOpenseaListing(token: Contract, tokenId: BigNumber, price: BigNumber, seller_?: Wallet): Promise<OpenseaListing> {
    ...
    async function buyOpenseaListing(orderParams: OpenseaOrderParams): Promise<TransactionReceiptWithEvents> {
        noise('\tBuying OpenSea listing...');
        const totalPrice =
            orderParams.consideration.reduce((a, c) => a.add(c.startAmount), ZERO);
        return await mineTx(opensea.connect(buyer).fulfillOrder(
    ...
    async function bidOnZoraListing(auctionId: BigNumber, price: BigNumber): Promise<void> {
        await mineTx(zora.connect(buyer).createBid(
            auctionId,
            price,
            { value: price },
        ));
    ...
    async function contributeEvenly(cf: Contract, contributorWallets: Wallet[], totalContribution: BigNumber) {
        noise(`\tContributing ${totalContribution.toString()} across ${contributorWallets.length} contributors...`);
