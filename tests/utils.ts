...
export function describeFork(name: string, body: (forkProvider: MockProvider) => void) {
  ...
  global.it = Object.assign((name: string, ...args: any[]) => {
    it(`${name} [⑃]`, ...args);
  }, global.it);
...
export async function increaseBalance(
  provider: MockProvider,
  address: string,
  extraBalance: BigNumber | number,
) {
  const bal = await provider.getBalance(address);
  await provider.send("evm_setAccountBalance", [
    address,
    ethers.utils.hexlify(bal.add(extraBalance)),
  ]);
}

export async function createUnlockedWallet(
  provider: MockProvider,
  address: string,
  signer: Signer,
): Promise<Contract> {
