...
contract DummyProposalExecutionEngine is IProposalExecutionEngine, ProposalStorage {
    ...
    function initialize(address oldImpl, bytes calldata initializeData) external {
        // Prevent old parties from configuring new options to maintain security guarantees.
        if (oldImpl != address(0)) return;

        ProposalStorage.ProposalEngineOpts memory opts = abi.decode(
            initializeData,
...
contract DummyTokenDistributor is ITokenDistributor {
    ...
    function wasFeeClaimed(Party, uint256) external view returns (bool) {}

    function hasPartyTokenIdClaimed(Party, uint256, uint256) external view returns (bool) {}

    function getRemainingMemberSupply(Party, uint256) external view returns (uint128) {}
}
...
contract TestablePartyGovernance is PartyGovernance {
    ...
    constructor(
        ...
        _initialize(govOpts, proposalEngineOpts, preciousTokens, preciousTokenIds);
    }
    ...
    function getProposalTotalVotingPower(
        uint256 proposalId
    ) external view returns (uint96 totalVotingPower) {
        (, ProposalStateValues memory values) = this.getProposalStateInfo(proposalId);
        return values.totalVotingPower;
    }
...
contract PartyGovernanceUnitTest is Test, TestUtils {
    ...
    function testProposalLifecycle_disabledWhenNoTotalVotingPower() external {
        (
            IERC721[] memory preciousTokens,
            uint256[] memory preciousTokenIds
        ...
        gov.propose(_createProposal(1), 0);
    }
    ...
    function testProposalLifecycle_worksDuringTotalVotingPowerUpdate() external {
        (
            IERC721[] memory preciousTokens,
            uint256[] memory preciousTokenIds
        ) = _createPreciousTokens(2);
        ...
        // undelegatedVoter2 has 75/100 intrinsic VP (delegated to no one/self)
        ...
        // Increase total voting power so that if the proposal did not use the
        // total voting power at the time of proposal and used the current total
        // voting power, it would fail.
        gov.increaseTotalVotingPower(100e18);
        // Global total voting power should be 200.
        assertEq(gov.getTotalVotingPower(), 200e18);
        // Proposal total voting power should still be 100.
        assertEq(gov.getProposalTotalVotingPower(proposalId), 100e18);
    ...
    function testDistribute_canOnlyDistributeIfPartyStarted() external {
        ...
        TestablePartyGovernance gov = _createGovernance(false, 0, preciousTokens, preciousTokenIds);
    ...
    function testDistribute_canOnlyDistributeIfDoesNotRequireVotes() external {
        ...
        TestablePartyGovernance gov = _createGovernance(
            true, // Set distribution to require votes.
            100e18,
            preciousTokens,
            preciousTokenIds
        );
