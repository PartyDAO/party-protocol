...
contract TokenDistributorUnitTest is Test, TestUtils {
    event DistributionCreated(Party indexed party, ITokenDistributor.DistributionInfo info);
    ...
    constructor() {
        ...
        party = Party(payable(address(new TestParty())));
    }
    ...
    function test_claimNative_cannotChangeDistributionInfo() external {
        ...
        for (uint256 i; i < 8; ++i) {
            ...
            if (i == 0) {
                ...
                di_.party = Party(payable(address(fakeParty)));
            } else if (i == 3) {
                ...
                di_.totalShares = uint96(_randomUint256() % type(uint96).max);
            }
            vm.expectRevert(
    ...
    function test_getDistributionHash() external {
        ITokenDistributor.DistributionInfo memory di = ITokenDistributor.DistributionInfo({
            ...
            totalShares: uint96(_randomUint256())
        });
