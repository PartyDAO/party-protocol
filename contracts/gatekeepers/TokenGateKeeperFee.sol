 ...
 */
contract TokenGateKeeperFee is IGateKeeper {
    // last gate id
    uint96 private _lastId;
    ContributionRouter immutable CONTRIBUTION_ROUTER;

    constructor(ContributionRouter contributionRouter) {
    ...
    function isAllowed(
        ...
        if (participant == address(CONTRIBUTION_ROUTER)) {
            participant = CONTRIBUTION_ROUTER.sender();
        }
