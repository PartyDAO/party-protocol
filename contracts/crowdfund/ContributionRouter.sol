...
contract ContributionRouter {
    ...
    event FeePerMintUpdated(uint256 oldFeePerMint, uint256 newFeePerMint);
    ...
    StorageSlot internal theSlot;

    constructor(address owner, uint96 initialFeePerMint) {
        OWNER = owner;
        theSlot.feePerMint = initialFeePerMint;
    }
    ...
    function setFeePerMint(uint96 newFeePerMint) external onlyOwner {
        emit FeePerMintUpdated(theSlot.feePerMint, newFeePerMint);

        theSlot.feePerMint = newFeePerMint;
    }
    ...
    fallback() external payable {
        uint256 feeAmount = theSlot.feePerMint;
        theSlot.caller = msg.sender;
