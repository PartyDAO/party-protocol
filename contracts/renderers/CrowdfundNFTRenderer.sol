...
contract CrowdfundNFTRenderer is IERC721Renderer, RendererCustomization {
    ...
    function generateSVG2(Color color) private view returns (string memory) {
        ...
        return string.concat(
            ...
            ';font-family:pixeldroidConsoleRegular,Console;font-size:48px}.s{fill:none;stroke:',
            generateColorHex(color, ColorType.PRIMARY),
    ...
    function getContribution(address owner) private view returns (string memory) {
        ...
        if (ethContributed < 0.01e18) {
            ...
            return string.concat((ethContributed / 1e18).toString(), ".", (ethContributed / 1e17 % 1e1).toString());
        } else if (ethContributed >= 10e18) {
            // Truncate value to 2 decimals
            return string.concat((ethContributed / 1e18).toString(), ".", (ethContributed / 1e16 % 1e2).toString());
        } else {
            // Truncate value to 3 decimals
            return string.concat((ethContributed / 1e18).toString(), ".", (ethContributed / 1e15 % 1e3).toString());
        }
    }
