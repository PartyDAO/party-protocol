...
yargs.command(
  ...
  async argv => {
    ...
    const artifactContents = await Promise.all(
      artifactPaths.map(p => fs.readFile(p, { encoding: "utf8" })),
    );
    const errorAbis = uniqifyAbi(
      artifactContents
        .map(a => JSON.parse(a).abi)
        .filter(v => !!v)
...
function serialize(v: any): any {
  ...
  if (typeof v === "object") {
    return Object.assign({}, ...v.entries().map(([ok, ov]: any[]) => ({ [ok]: serialize(ov) })));
  }
  return v;
}
